...
async def get_trains(
    ...
    total = await mongo.trains_collection.count_documents(query)
    cursor = mongo.trains_collection.find(query).skip(skip).limit(limit)
...
async def get_train(train_id: str):
    ...
    train = await mongo.trains_collection.find_one({"$or": [{"train_id": train_id}, {"_id": train_id}]})
    if not train:
...
async def create_train(train: Train):
    ...
    created_train = await mongo.trains_collection.find_one({"_id": train_dict["_id"]})
...
async def update_train(train_id: str, train_update: Train):
    ...
    existing = await mongo.trains_collection.find_one({"$or": [{"train_id": train_id}, {"_id": train_id}]})
    if not existing:
        raise HTTPException(
    ...
    if "_id" in update_data:
        del update_data["_id"]
    await mongo.trains_collection.update_one({"_id": existing.get("_id", train_id)}, {"$set": update_data})
    
    updated_train = await mongo.trains_collection.find_one({"_id": existing.get("_id", train_id)})
...
async def delete_train(train_id: str):
    """Delete a train"""
    result = await mongo.trains_collection.delete_one({"$or": [{"train_id": train_id}, {"_id": train_id}]})
